...
class Game{
   ...
   private double drawReward = 0;
   private int loseReward = -1;
...
//          System.out.println();
   ...
   public void AIPlayRandom(double epsilon){
      board = "         ";
      ArrayList<String> statesVisited = new ArrayList<String>();
      ArrayList<Integer> stateActionPairs = new ArrayList<Integer>();
      boolean over = false;
...
//          System.out.println();
   ...
   public void AITest(){
      ...
      while(!over){
         ...
         for(int j = 0; j < qTableX.size(); j++){
            ...
            if(isSame((int)Math.floor(x[0]), hash(board))[0] == 1){
               visitedBefore = true;
               break;
            }
         }
   ...
   public void train(){
      //qwe
      int q1 = hash("XXOOXXXOO");
      int q2 = hash("OXOOXXXOX");
      System.out.println(Arrays.toString(isSame(q1, q2)));
      /*int q1 = hash("XO  X   X");
      int q2 = hash(" OX X X  ");
      int q3 = hash("  X X XO ");
      int q4 = hash("X   X  OX");
      ...
      int isTrueCount = 0;
      int isFalseCount = 0;
      for(int i1 = 0; i1 < q0.length; i1++){
         for(int j1 = 0; j1 < q0.length; j1++){
            if(!(i1 == j1)){
               if(isSame(q0[i1], q0[j1])[0] == 1){
                  isTrueCount++;
               } else {
                  isFalseCount++;
               }
      ...
      System.out.println("True: " + isTrueCount);
      System.out.println("False: " + isFalseCount);
      */            
               
      try{
         double gamesPlayed = 1.0;
         PrintStream original = System.out;
         System.setOut(new PrintStream(new FileOutputStream("data.txt", true)));
         System.out.println("Games Played (Thousands),X Win/Draw Percentage");
         boolean testRun = true;
         for(int i = 0; i < 100000; i++){
            //if(i % 2 == 0){
               this.AIPlay(1 * Math.pow(0.999987, gamesPlayed));
            //}
            //else{
               //this.AIPlayRandom(1 * Math.pow(0.999987, gamesPlayed));
            //}
            gamesPlayed++;
            if(i % 1000 == 0){
               DecimalFormat format = new DecimalFormat("#.00");
               System.out.print("Games Played: " + i + ", Q-Table size: " + qTableX.size());
               /*
               ...
               */
               ...
               for(int j = 0; j < 1000; j++){
                  int a5 = randomOWins;
                  ArrayList<String> a6 = this.AITest2();
                  /*
                  if(randomOWins > a5 && testRun == true && i > 30000){
                     for(int k = 0; k < a6.size(); k++){
                        display(hash(a6.get(k)));
                        System.out.println();
                     }
                     testRun = false;
                  }
               ...
               System.out.print(", X Win/Draw Percentage: " + format.format(((double)(randomXWins + randomDraws))/1000 * 100) + "%");
               System.out.println(", X Wins: " + randomXWins + ", O Wins: " + randomOWins + ", Draws: " + randomDraws);
               //System.out.println((i/1000) + "," + format.format((randomXWins + randomDraws)/10.0));
   ...
   public void updateQTable(ArrayList<Integer> sAPairs, double reward, String player){
      if(player.equals("X")){
         for(int i = sAPairs.size() - 1; i > 0; i -= 2){
            ...
            for(int j = 0; j < qTableX.size(); j++){
               Double[] x = qTableX.get(j);
               if(isSame((int)Math.floor(x[0]), sAPairs.get(i - 1))[0] == 1){
                  contains = true;
                  qIndex = j;
                  break;
               }
   ...
   public int bestMove(int hashedBoard, ArrayList<Integer> moveSet1, String player){
      if(player.equals("X")){
         State state = new State(unHash(hashedBoard), player);
         ...
         for(int i = 0; i < qTableX.size(); i++){
            Double[] x = qTableX.get(i);
            if(isSame((int)Math.floor(x[0]), hashedBoard)[0] == 1){
               x1 = x;
               state = new State(unHash((int)Math.floor(x[0])), player);
         ...
         int[] vars = isSame((int)Math.floor(x1[0]), hashedBoard);
         String boardVar = "012345678";
         String tempBoard = "";
         int ret = -1;
         if(Arrays.equals(vars, new int[]{1, 0, 0})){ //no rotation, no inversion
            tempBoard += boardVar;
            ret = Integer.parseInt(tempBoard.substring(index, index + 1));
         }
         else if(Arrays.equals(vars, new int[]{1, 1, 0})){ //90 degree rotation, no inversion
            tempBoard += rotate(boardVar);
            ret = Integer.parseInt(tempBoard.substring(index, index + 1));
         }
         else if(Arrays.equals(vars, new int[]{1, 2, 0})){ //180 degree rotation, no inversion
            boardVar = rotate(boardVar);
            tempBoard += rotate(boardVar);
            ret = Integer.parseInt(tempBoard.substring(index, index + 1));
         }
         else if(Arrays.equals(vars, new int[]{1, 3, 0})){ //270 degree rotation, no inversion
            boardVar = rotate(boardVar);
            boardVar = rotate(boardVar);
            tempBoard += rotate(boardVar);
            ret = Integer.parseInt(tempBoard.substring(index, index + 1));
         }
         else if(Arrays.equals(vars, new int[]{1, 0, 1})){ //no rotation, horizontal inversion
            tempBoard += boardVar.substring(6) + boardVar.substring(3, 6) + boardVar.substring(0, 3);
            ret = Integer.parseInt(tempBoard.substring(index, index + 1));
         }
         else if(Arrays.equals(vars, new int[]{1, 1, 1})){ //90 degree rotation, horizontal inversion
            boardVar = rotate(boardVar);
            tempBoard += boardVar.substring(6) + boardVar.substring(3, 6) + boardVar.substring(0, 3);
            ret = Integer.parseInt(tempBoard.substring(index, index + 1));
         }
         else if(Arrays.equals(vars, new int[]{1, 0, 2})){ //no rotation, vertical inversion
            tempBoard += invert(boardVar.substring(0, 3)) + invert(boardVar.substring(3, 6)) + invert(boardVar.substring(6));
            ret = Integer.parseInt(tempBoard.substring(index, index + 1));
         }
         else if(Arrays.equals(vars, new int[]{1, 1, 2})){ //90 degree rotation, vertical inversion
            boardVar = rotate(boardVar);
            tempBoard += invert(boardVar.substring(0, 3)) + invert(boardVar.substring(3, 6)) + invert(boardVar.substring(6));
            ret = Integer.parseInt(tempBoard.substring(index, index + 1));
         }
         ...
         int[] vars = isSame((int)Math.floor(x1[0]), hashedBoard);
         String boardVar = unHash(hashedBoard).substring(0, index) + "C" + unHash(hashedBoard).substring(index + 1);
         String tempBoard = "";
         int ret = -1;
         if(Arrays.equals(vars, new int[]{1, 0, 0})){ //no rotation, no inversion
            tempBoard += boardVar;
            ret = tempBoard.indexOf("C");
         }
         else if(Arrays.equals(vars, new int[]{1, 1, 0})){ //90 degree rotation, no inversion
            tempBoard += rotate(boardVar);
            ret = tempBoard.indexOf("C");
   ...
   public int atWinState(int hashedBoard, String player){
      String opponent = "";
      if(player.equals("X")){
         opponent += "O";
      }
      ...
      for(int i = 0; i < winningPatterns.size(); i++){
         ...
         int b = Integer.parseInt(winningPatterns.get(i).substring(1, 2));
         int c = Integer.parseInt(winningPatterns.get(i).substring(2));
         String board = unHash(hashedBoard);
         String d = board.substring(a, a+1) + board.substring(b, b+1) + board.substring(c, c+1);
         if(d.equals(" " + player + player)){
            return a;
         }
         else if(d.equals(player + " " + player)){
            return b;
         }
         else if(d.equals(player + player + " ")){
            return c;
      ...
      for(int j = 0; j < winningPatterns.size(); j++){
         int a = Integer.parseInt(winningPatterns.get(j).substring(0, 1));
         int b = Integer.parseInt(winningPatterns.get(j).substring(1, 2));
         int c = Integer.parseInt(winningPatterns.get(j).substring(2));
         String board = unHash(hashedBoard);
         String d = board.substring(a, a+1) + board.substring(b, b+1) + board.substring(c, c+1);
         if(d.equals(" " + opponent + opponent)){
            return a;
         }
         else if(d.equals(opponent + " " + opponent)){
            return b;
         }
         else if(d.equals(opponent + opponent + " ")){
            return c;
   ...
   public static int[] isSame(int hb1, int hb2){
      //First index of array: 1 is true, 0 is false
      //Second index of array (rotation): 0 is no rotation, 1 is 90 degrees clockwise, 
      //2 is 180 degrees clockwise, 3 is 270 degrees clockwise
      //Third index of array (inversion): 0 is no inversion, 1 is horizontal inversion, 
      //2 is vertical inversion
      //exact copy is {1, 0, 0}
      int[] ret;
      int hb1temp = hb1;
      int hb2temp = hb2;
      ...
      if((xCount1 != xCount2) || (oCount1 != oCount2)){
         ret = new int[]{0, 0, 0};
         return ret;
      }
      if(hb1 == hb2){
      ...
      String b1 = unHash(hb1);
      String b2 = unHash(hb2);
      ...
      for(int i = 0; i < x.length; i++){ 
         //Inversion Invariance Test
         //Horizontality
         if(x[i].substring(0, 3).equals(b2.substring(6)) &&
            x[i].substring(3, 6).equals(b2.substring(3, 6)) &&
            x[i].substring(6).equals(b2.substring(0,3))){
            ret = new int[]{1, i, 1};
            return ret;
         }
         //Verticality
         if(invert(x[i].substring(0, 3)).equals(b2.substring(0, 3)) &&
            invert(x[i].substring(3, 6)).equals(b2.substring(3, 6)) && 
            invert(x[i].substring(6)).equals(b2.substring(6))){
            ret = new int[]{1, i, 2};
            return ret;
         }
      }
   ...
   public static String rotate(String x){
      ...
      String[] y = x.split("");
      ret += y[6] + y[3] + y[0] + y[7] + y[4] + y[1] + y[8] + y[5] + y[2];
      return ret;
   }  
